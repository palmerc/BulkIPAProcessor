...
ORIGINAL_IPA_FOLDER="/Users/palmerc/Music/iTunes/iTunes Media/Mobile Applications"
DECRYPTED_IPA_FOLDER="/Users/palmerc/Hacking/Decrypted Apps"

IOS_DEPLOY="/usr/local/bin/ios-deploy"
PLISTBUDDY="/usr/libexec/PlistBuddy"
FRIDA_DUMP_PY="/Users/palmerc/Hacking/frida-ios-dump/dump.py"
...
for ORIGINAL_IPA in "${ORIGINAL_IPAS[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        FRIDA_DUMP_CMD="${FRIDA_DUMP_PY} ${APP_BUNDLE_ID}"
        echo "${FRIDA_DUMP_CMD}"
        eval "${FRIDA_DUMP_CMD}"
    fi
